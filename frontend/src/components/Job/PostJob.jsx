...
const PostJob = () => {
  ...
  const handleJobPost = async(e) => {
    ...
    await axios.post("http://localhost:4000/api/v1/job/post", fixedSalary.length >= 4 ? {title, description, category, country, city, location, fixedSalary} :{title, description, category, country, city, location, salaryFrom, salaryTo}, { withCredentials: true ,
      headers: {
        ...
        <form onSubmit={handleJobPost}>
          ...
          <div className='salary_wrapper'>
            <select value={salaryType} onChange={(e) => setSalaryType(e.target.value)}>
              ...
              <option value="Rnaged Salary">Ranged Salary</option>
            </select>
            <div>
              {
